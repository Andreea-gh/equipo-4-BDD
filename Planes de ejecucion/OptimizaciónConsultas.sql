...
-- SOBRE LOS INDICES: Decidimos crear indices agrupados ya que son mas rapidos que los no agrupados, ademas de que los no agrupados
-- empeoran cuando hay demasiados datos. Creamos un indice sobre la columna ENTIDAD_RES porque sus datos no se repiten, también hicimos
-- pruebas sobre ENTIDAD_UM, MUNICIPIO_RES pero no notamos una diferencia en el rendimiento por lo menos en los planes de ejeucion no vimos
-- una diferencia en porcentaje

-- Creamos un indice agrupado sobre la columna ENTIDAD_RES porque los datos en dicha columna
-- no se repiten ademas de que en las consultas el order by recae sobre el atributo ENTIDAD_RES
...
-- Al igual que en la consulta anterior podemos crear un indice sobre 
-- ENTIDAD_RES, solo que también podriamos hacerlo sobre ENTIDAD_UM y 
...
-- De manera similiar al caso anterior creamos un indice agrupado solo que ahora sobre la columna MUNICIPO_RES
-- hacerlo sobre las otras columnas de la consulta no tiene una diferencia visible sobre el rendimiento
